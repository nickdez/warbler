...
def users_show(user_id):
    ...
    likes = [message.id for message in g.user.likes] if g.user else []
    ...
    if form.validate_on_submit():
        if User.authenticate(user.username, form.password.data):
            ...
            for key, value in new_data.items():
                if value == "":
                    setattr(user, key, getattr(user, key))
...
def remove_like_from_index(message_id):
    if not g.user:
        flash("Access unauthorized.", "danger")
        return redirect("/")
